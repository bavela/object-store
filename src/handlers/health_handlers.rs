...
pub async fn readyz(State(service): State<StorageService>) -> impl IntoResponse {
    ...
    let disk_check = match fs::write(&tmp_name, b"readyz").await {
        Ok(_) => match fs::read(&tmp_name).await {
            Ok(bytes) => {
                if bytes == b"readyz" {
                    // try to remove the temp file; ignore removal error but report if it happens
                    match fs::remove_file(&tmp_name).await {
                        Ok(_) => (true, None::<String>),
                        Err(e) => (true, Some(format!("could not remove tmp file: {}", e))),
                    }
                } else {
                    // content mismatch
                    let _ = fs::remove_file(&tmp_name).await; // best-effort cleanup
